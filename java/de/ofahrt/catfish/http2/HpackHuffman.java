 ...
 * blocks. Encoding is not implemented (we emit raw strings).
 */
final class HpackHuffman {

  // Huffman code table from RFC 7541 Appendix B.
  // Each entry: [code, bit_length]. Codes are MSB-aligned within their bit length.
  ...
  // Decode tree: each node is either a branch (children[0..1]) or a leaf (symbol).
  // We build a binary tree from the Huffman codes and walk it bit by bit.
  ...
  static {
    // Pre-allocate tree nodes. Worst case ~1024 nodes for 257 symbols with max 30-bit codes.
    int capacity = 2048;
  ...
  /** Decodes a Huffman-encoded string. Throws IllegalArgumentException on invalid encoding. */
  static String decode(byte[] data, int offset, int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      for (int bit = 7; bit >= 0; bit--) {
        ...
        node = direction == 0 ? LEFT[node] : RIGHT[node];
        bitsRead++;

        if (SYMBOL[node] != BRANCH) {
          int sym = SYMBOL[node];
          if (sym == 256) {
            throw new IllegalArgumentException("EOS symbol in Huffman-encoded string");
          }
          out.write(sym);
          node = 0;
          bitsRead = 0;
    ...
    // Remaining bits must be the MSBs of an EOS padding (all 1s) and at most 7 bits.
    if (bitsRead > 7) {
