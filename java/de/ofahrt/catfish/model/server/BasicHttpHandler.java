...
public final class BasicHttpHandler implements HttpHandler {
  ...
  public void handle(Connection connection, HttpRequest request, HttpResponseWriter responseWriter)
    ...
    if (request.getHeaders().get(HttpHeaderName.EXPECT) != null) {
      ...
      handleTrace(request, responseWriter);
        ...
        && !HttpMethodName.OPTIONS.equals(request.getMethod())) {
  ...
  protected void handleTrace(HttpRequest request, HttpResponseWriter responseWriter)
      throws IOException {
    try (OutputStreamWriter writer =
        new OutputStreamWriter(
            responseWriter.commitStreamed(
                StandardResponses.OK.withHeaderOverrides(TRACE_HEADERS)))) {
      writer
