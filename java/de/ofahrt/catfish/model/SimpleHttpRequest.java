...
public final class SimpleHttpRequest implements HttpRequest {
  ...
  public static class Builder {
    ...
    public HttpRequest build() throws MalformedRequestException {
      ...
      // A Content-Length (including 0) or Transfer-Encoding declares a body, which is always
      // represented as a Body object — an empty one for Content-Length: 0. Callers that have no
      // payload bytes but a framing header (e.g. the HTTP/2 dispatch path) must attach an empty
      // body, so the invariant stays simple: a framing header is present iff a body object is.
