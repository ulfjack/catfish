...
final class Emit {
  ...
  String lean() {
    ...
    // Simple class name (drop the package) keeps the Lean namespace readable.
    String simpleClass = cf.thisClass.substring(cf.thisClass.lastIndexOf('.') + 1);
    out("import Jvm.Semantics");
    for (String s : leanImports) out("import " + s);
    out("");
    out("namespace Generated." + simpleClass + "." + method);
  ...
  private void invDef(Vcg.Ins i) {
    ...
    if (i.stackReturn) {
      // `return <expr>;`: the returned value is the sole operand-stack element.
      out("  (s.stk = [" + i.specString.strip() + "])");
    } else {
