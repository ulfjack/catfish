...
final class Emit {
  ...
  final List<String[]> specs = new ArrayList<>(); // {module, namespace} to import + open
  ...
  String lean() {
    ...
    for (String[] s : specs) out("import " + s[0]);
    out("");
    out("namespace Generated." + cf.thisClass + "." + method);
    StringBuilder opens = new StringBuilder("open Jvm");
    for (String[] s : specs) if (s.length > 1 && !s[1].isEmpty()) opens.append(' ').append(s[1]);
    out(opens.toString());
