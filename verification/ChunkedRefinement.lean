...
/-
  ...
  and the 15-digit cap (`St` follows RFC `1*HEXDIG` with no cap).  The DATA phase
  is modelled one byte at a time; `advance` bulk-forwards the same span.
-/
...
def advanceStep (cs : ClassState) (b : Int) : ClassState :=
  match cs.state with
  | .SIZE =>
      match digitVal b with
      | some d =>
          let sz := cs.size * 16 + d.toNat
          if (sz : Int) ≤ Jvm.MAXI then { cs with size := sz, digits := cs.digits + 1 }
...
theorem advanceStep_wf (cs : ClassState) (b : Int) (h : Wf cs) : Wf (advanceStep cs b) := by
...
theorem advanceStep_refines (cs : ClassState) (b : Int) (h : Wf cs) :
    α (advanceStep cs b) = chunkStep (α cs) b := by
  obtain ⟨st, sz, dg, dl⟩ := cs
  simp only [Wf] at h
  cases st
  case SIZE =>
    ...
    · subst hdg
      ...
      | some d =>
        ...
        have hdn : (d.toNat : Int) = d := Int.toNat_of_nonneg hd0
        simp only [advanceStep, hd]
        rw [if_pos (by simp only [Jvm.MAXI]; omega)]
        simp [α, chunkStep, hd]
      | none =>
        have hadv : advanceStep ⟨CState.SIZE, 0, 0, dl⟩ b = ⟨CState.ERROR, 0, 0, dl⟩ := by
    ...
    · rw [show α ⟨CState.SIZE, sz, dg, dl⟩ = St.size sz by simp [α, hdg]]
      ...
      | none =>
        by_cases hb : b = CR
        · have hadv : advanceStep ⟨CState.SIZE, sz, dg, dl⟩ b = ⟨CState.SIZE_CR, sz, dg, dl⟩ := by
        ...
        · have hadv : advanceStep ⟨CState.SIZE, sz, dg, dl⟩ b = ⟨CState.ERROR, sz, dg, dl⟩ := by
...
theorem foldl_advanceStep_refines (cs : ClassState) (h : Wf cs) (bs : List Int) :
    α (bs.foldl advanceStep cs) = bs.foldl chunkStep (α cs) := by
  induction bs generalizing cs with
  | nil => rfl
  | cons b bs ih =>
    simp only [List.foldl_cons]
    rw [ih (advanceStep cs b) (advanceStep_wf cs b h), advanceStep_refines cs b h]
...
theorem advance_refines_decode (bs : List Int) :
    α (bs.foldl advanceStep initial) = decode bs := by
  rw [foldl_advanceStep_refines initial initial_wf bs]; rfl
...
example : α ([49, 13, 10, 65, 13, 10, 48, 13, 10].foldl advanceStep initial) = St.done := by
